...
///
/// Pass the same token to several operations to cancel them together (for
/// example when a host is disposed). Backed by a [Completer]: completing it
/// (via [cancel]) signals cancellation to anything awaiting [whenCancel].
/// Transport-agnostic — wire [whenCancel] into your client's abort trigger.
class CancelToken {
  ...
  /// each detaches on completion (see [link]) so it never grows with total
  /// requests.
  ...
  /// Links [child] so it cancels together with this token; returns an unlink
  /// callback.
